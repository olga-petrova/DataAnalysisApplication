...
class RemoteMatrix{
    ...
    public function process(){
        ...
        $resultItems = $results->calculate($req['leftAxis'], $req['topAxis']);
...
class Results {
    ...
    public function calculate($leftAxis, $topAxis){
          $sqlSelect = array();

          foreach($this->dimensions as $dimension){
              $sqlSelect[] = "round({$dimension['aggregator']}({$dimension['dataIndex']})) as {$dimension['id']}";
          }
          foreach($leftAxis as $dimension){
              $sqlSelect[] = "{$dimension['dataIndex']}";
          }

          foreach($topAxis as $dimension){
              $sqlSelect[] = "{$dimension['dataIndex']}";
          }
          ...
          foreach($this->items as &$item){
            ...
            if ($isTopTotal) {
              $aggragatedItems = 0;
              foreach($tempResult as $tempItem){
                  $isCorrectItem = count($item['leftFields']) > 0 ? 1: 0;
                  foreach($item['leftFields'] as $fKey => $fValue){
                      if($tempItem[$fKey] != $fValue) {
                          $isCorrectItem = 0;
                      }
